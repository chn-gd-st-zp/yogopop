...
{
    private static WSInstance _instance;
    private static WSInstance Instance => _instance ?? throw new Exception("使用前请初始化 WSClient.Initialization(...);");

    /// <summary>
    /// 初始化
    /// </summary>
    /// <param name="options"></param>
    public static void Initialization(WSOptions options) { _instance = new WSInstance(options); }
    ...
    public static void SendMessage(Guid receiveClientId, object message, bool receipt = false) => Instance.SendMessage(Unique.GetGUID().ToGuid(), new Guid[] { receiveClientId }, message, receipt);
    ...
    /// <param name="message">消息</param>
    /// <param name="receipt">是否回执</param>
    public static void SendMessage(Guid senderClientId, Guid receiveClientId, object message, bool receipt = false) => Instance.SendMessage(senderClientId, new Guid[] { receiveClientId }, message, receipt);
    ...
    /// <param name="message">消息</param>
    /// <param name="receipt">是否回执</param>
    public static void SendMessage(IEnumerable<Guid> receiveClientId, object message, bool receipt = false) => Instance.SendMessage(Unique.GetGUID().ToGuid(), receiveClientId, message, receipt);
    ...
    /// <param name="chan">群聊频道名</param>
    /// <param name="message">消息</param>
	public static void SendChanMessage(string chan, object message) => Instance.SendChanMessage(Unique.GetGUID().ToGuid(), chan, message);
